    ...
    public TelnetUtil(@Value("#{'${router.ip:}'.split(',')}") String[] ip,
        ...
        try {
            ...
            for (int i = 1; i <= ip.length; i++) {
                System.out.println(ip[i - 1] + " " + port + " " + user + " " + password + " " + enablePassword);
...
//        }
        telnet[routerId].sendCommand(command);
        s = telnet[routerId].sendCommand(command);
    ...
    public String sendCommandByRouter3(String command) {
        String s = "";
        telnet[3].sendCommand(command);
        s = telnet[3].sendCommand(command);
        System.out.println(s);
        return s;
    }
